...
class DatabaseBackup {
    ...
    checkLastBackup() {
        ...
        if (
            this.currentDate.day > lastBackup.day ||
            this.currentDate.month > lastBackup.month ||
            this.currentDate.year > lastBackup.year
        ) {
            const newDate = {};
            ...
            fs.writeFileSync(
                './_assets/functions/DatabaseBackup/last_backup.json',
                JSON.stringify(newDate, null, 4)
            );
    ...
    dumb(table = null) {
        cp.exec(
            `mysqldump -u ${process.env.DB_USER} -p'${process.env.DB_PWD}' ${
                process.env.DB_NAME
            } --no-tablespaces --no-data ${table ? table : ''} > ./${
                table ? './_backup/tables/' + table + '-' : './_backup/database/'
        ...
        cp.exec(
            `mysqldump -u ${process.env.DB_USER} -p'${process.env.DB_PWD}' ${
                process.env.DB_NAME
            } --no-tablespaces --no-create-info --no-create-db ${table ? table : ''} > ./${
                table ? './_backup/tables/' + table + '-' : './_backup/database/'
            }${this.getDataPath()}`,
            (error, stdout, stderr) => {
                if (error !== null) {
    ...
    pushToRepo() {
        setTimeout(() => {
            cp.exec(
                ` git pull && git add ${this.getDataPath()} && git add ${this.getStructurePath()} && git commit -m "Database backup created due server restart" && git push`,
                (error, stdout, stderr) => {
                    console.log(stdout);
                    console.log(stderr);
                    if (error !== null) {
