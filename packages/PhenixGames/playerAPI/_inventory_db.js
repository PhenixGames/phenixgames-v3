 ...
 */
module.exports.updatePlayerInventoryToDatabase = async ({
    player_id,
    items
}) => {
    return new Promise(async (resolve, reject) => {
        await database.query(`UPDATE pg_user_inventory SET items = ? WHERE user_id = ?`, [JSON.stringify(items), player_id])
            .then(() => {
                return resolve({
                    error: false,
 ...
 */
module.exports.getItemConfig = async ({
    item_id
}) => {
    return await database.query('SELECT * FROM pg_item_config WHERE item_id = ?', [item_id])
        .then(res => {
            if(res.length > 0) {
                return {
                    error: false,
